...
export default function Noithat() {
    const [noithat, setNoithat] = useState([]);
    useEffect(() => {
        getNoithat();
    }, []);

    async function getNoithat() {
        let result = await fetch("http://127.0.0.1:8000/api/noithat_product");
   ...
   <div>
       <Col>
            <div className="titles">
                <h1><b>Sản phẩm nội thất cũ</b></h1>
            </div>
            <CardGroup>
            {noithat.map((product,key) => (
