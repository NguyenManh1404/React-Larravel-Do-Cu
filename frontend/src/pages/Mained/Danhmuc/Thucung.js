...
export default function Thucung() {
    const [thucung, setThucung] = useState([]);
    useEffect(() => {
        getThucung();
    }, []);

    async function getThucung() {
        let result = await fetch("http://127.0.0.1:8000/api/thucung_product");
   ...
   <div>
       <Col>
            <div className="titles">
                <h1><b>Mua bán thú cưng</b></h1>
            </div>
            <CardGroup>
            {thucung.map((product,key) => (
